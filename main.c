...
unsigned int get_mcycle()   { unsigned int val; asm volatile("csrr %0, mcycle" : "=r"(val)); return val; }
unsigned int get_minstret() { unsigned int val; asm volatile("csrr %0, minstret" : "=r"(val)); return val; }
unsigned int get_hpm3()     { unsigned int val; asm volatile("csrr %0, mhpmcounter3" : "=r"(val)); return val; }
unsigned int get_hpm4()     { unsigned int val; asm volatile("csrr %0, mhpmcounter4" : "=r"(val)); return val; }
unsigned int get_hpm5()     { unsigned int val; asm volatile("csrr %0, mhpmcounter5" : "=r"(val)); return val; }
unsigned int get_hpm6()     { unsigned int val; asm volatile("csrr %0, mhpmcounter6" : "=r"(val)); return val; }
unsigned int get_hpm7()     { unsigned int val; asm volatile("csrr %0, mhpmcounter7" : "=r"(val)); return val; }
unsigned int get_hpm8()     { unsigned int val; asm volatile("csrr %0, mhpmcounter8" : "=r"(val)); return val; }
unsigned int get_hpm9()     { unsigned int val; asm volatile("csrr %0, mhpmcounter9" : "=r"(val)); return val; }
...
void draw_menu_ui(int active_switch) {
    ...
    for(i = 0; i < 5; i++) {
        int x = 40 + (i * 50); int y = 100; int size = 30;
...
void timer_wait_one_second() {
    *TIMER_CONTROL = 0; *TIMER_STATUS = 0; *TIMER_LOW = 0xC380; *TIMER_HIGH = 0x01C9; *TIMER_CONTROL = 0x4; 
            ...
            while (1) {
                if (!game_over && !level_complete) {
                    ...
                    if (benchmark_mode) {
                        s_cyc = get_mcycle();
                        s_inst= get_minstret();
                        s_h3  = get_hpm3(); s_h4 = get_hpm4(); s_h5 = get_hpm5();
                        s_h6  = get_hpm6(); s_h7 = get_hpm7(); s_h8 = get_hpm8(); s_h9 = get_hpm9();
                    }

                    /* --- LOGIC START --- */
                    button_pressed = (*BUTTONS & 1); 
                    if (button_pressed && is_grounded) { cube_dy = JUMP_FORCE; is_grounded = 0; }
                    cube_y += cube_dy; cube_dy += GRAVITY; 
                    if (cube_y > FLOOR_Y - cube_size) { cube_y = FLOOR_Y - cube_size; cube_dy = 0; is_grounded = 1; }
                    scroll_x += current_speed;

                    for (i = 0; i < MAX_OBJS; i++) {
                        int obj_x = current_level_data[i].x; int obj_y = current_level_data[i].y; int type = current_level_data[i].type;
                        int screen_x;
                        if (obj_x == -1) break; 
                        screen_x = obj_x - scroll_x;
                        
                        if (screen_x > -50 && screen_x < 350) {
                            int hit = 0; int landed = 0;
                            if (type == TYPE_SPIKE) hit = check_collision_tri(cube_x, cube_y, cube_size, screen_x, obj_y, 20);
                            else if (type == TYPE_WALL) hit = check_collision_box(cube_x, cube_y, cube_size, screen_x, obj_y, 20, 20);
                            else if (type == TYPE_TRAP) hit = check_collision_box(cube_x, cube_y, cube_size, screen_x, obj_y, 20, 20);
                            else if (type == TYPE_PLAT) {
                                if (check_collision_box(cube_x, cube_y, cube_size, screen_x, obj_y, 40, 20)) {
                                    if (cube_dy >= 0 && (cube_y + cube_size) <= (obj_y + 12)) landed = 1;
                                    else hit = 1; 
                                }
                            }
                            if (landed) { cube_y = obj_y - cube_size; cube_dy = 0; is_grounded = 1; }
                            if (hit) game_over = 1;
                        }
                    }
                    ...
                    if (benchmark_mode) {
                        e_cyc = get_mcycle();
                        e_inst= get_minstret();
                        e_h3  = get_hpm3(); e_h4 = get_hpm4(); e_h5 = get_hpm5();
                        e_h6  = get_hpm6(); e_h7 = get_hpm7(); e_h8 = get_hpm8(); e_h9 = get_hpm9();
                ...
                /* Draw */
                if (game_over) clear_screen(COLOR_DEATH); 
                else if (level_complete) clear_screen(COLOR_WIN);   
                else clear_screen(COLOR_BG);    
                ...
                if (!game_over && !level_complete) {
                    ...
                    for (i = 0; i < MAX_OBJS; i++) {
                        ...
                        if (screen_x > -20 && screen_x < 320) {
                            if (type == TYPE_SPIKE) draw_triangle(screen_x, obj_y, 20, COLOR_SPIKE);
                            else if (type == TYPE_WALL) draw_rect(screen_x, obj_y, 20, 20, COLOR_WALL);
                            else if (type == TYPE_TRAP) draw_rect(screen_x, obj_y, 20, 20, COLOR_TRAP);
                            else if (type == TYPE_PLAT) draw_rect(screen_x, obj_y, 40, 20, COLOR_PLAT); 
                        }
                    }
                ...
                *(VGA_CTRL + 1) = (unsigned int)VGA + (back_buffer_index ? (SCREEN_W * SCREEN_H) : 0);
                ...
                if (game_over || level_complete) {
                    print("\n\n=== LEVEL FINISHED ===\n");
                    if (level_complete) print("RESULT: WIN\n"); else print("RESULT: GAME OVER\n");
                    print("Total Cycles: "); print_dec(total_cycles); print("\n");
                    print("Total Instr : "); print_dec(total_inst);   print("\n");
                    print("Mem Instr   : "); print_dec(total_mem);    print("\n");
                    print("I-Cache Miss: "); print_dec(total_imiss);  print("\n");
                    print("D-Cache Miss: "); print_dec(total_dmiss);  print("\n");
                    print("I-Cache Stal: "); print_dec(total_istall); print("\n");
                    print("D-Cache Stal: "); print_dec(total_dstall); print("\n");
                    print("Hazard Stall: "); print_dec(total_haz);    print("\n");
                    print("ALU Stall   : "); print_dec(total_alu);    print("\n");
                    print("======================\n");
